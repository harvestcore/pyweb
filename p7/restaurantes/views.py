...
class RestBusqueda2View(TemplateView):
    template = 'restaurantes/restaurantes2.html'
    ...
    def post(self, request):
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                if form.cleaned_data.get('name') != "":
                    name = form.cleaned_data.get('name')
                    rest = Restaurantes.getIntervaloByName(0, 10, name)
...
class IntervaloView(TemplateView):
    def get(self, request, ini = 0, cantidad = 10, nombre = ""):
        if request.method == 'GET':
            datos = Restaurantes.getIntervaloByName(int(ini), int(cantidad), str(nombre))
            totalelem = len(Restaurantes.getListByName(nombre))
            page = int(ini) / int(cantidad)
            data = {
                'data': datos,
                'totalelements': totalelem,
                'pageelem': int(cantidad),
                'fetched': len(datos),
                'page': page
...
class StatisticView(TemplateView):
    template = 'restaurantes/statistics.html'
    def get(self, request):
...
def restStat(request, name=""):
    if request.method == 'GET':
        ...
        percentage = round((len(rest)/allrest) * 100, 5)
