...
class Restaurantes:
    ...
    def getIntervalo(min_, max_):
        datalist = []
        data = restaurantes.find({}).skip(min_).limit(max_)
    ...
    def getIntervaloByName(min_, max_, name):
        datalist = []
        data = restaurantes.find({'name': { "$regex": ".*" + name + ".*", "$options": "-i" }}).skip(min_).limit(max_)
